...
namespace Wasteland::Core
{
    ...
    MouseMode operator!(const MouseMode& mode)
    {
        switch (mode)
        {
    ...
    public:
        ...
        {
            int button = static_cast<int>(code);
            bool wasPressed = previousMouseStates[button];
            bool isPressed = currentMouseStates[button];
        ...
        void Update()
        {
            ...
            Vector<float, 2> newPos{ static_cast<float>(x), static_cast<float>(y) };
    ...
    private:
        ...
        {
            InputManager::GetInstance().scrollDelta = static_cast<float>(yoffset);
        }
